...
async def test_send_verification_email_success(test_app: AsyncClient, create_test_user):
    ...
    async with DependencyOverrider(app, overrides={AWSSesRepository.send_verification_email: override_send_email_dependency_success}):
        token = await test_app.post("/users/login/", json=payload)
        headers = {'Authorization': 'Bearer ' + token.json().get("access_token")}
        response = await test_app.get("/users/account/verify_email/", headers=headers)
...
async def test_send_verification_email_processing_exception(test_app: AsyncClient, create_test_user):
    ...
    async with DependencyOverrider(app, overrides={AWSSesRepository.send_verification_email: override_send_email_dependency_exception}):
        token = await test_app.post("/users/login/", json=payload)
        headers = {'Authorization': 'Bearer ' + token.json().get("access_token")}
        response = await test_app.get("/users/account/verify_email/", headers=headers)
